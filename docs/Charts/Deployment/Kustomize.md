...
The contents of ingress-patch.yaml:  
```
- op: replace
...
```
helm template xnat ais/xnat > all.yaml && helm upgrade xnat ais/xnat -i -f values.yaml -nxnat --post-renderer=./hook.sh
```
